...
if __name__ == "__main__":
    ...
    df_comp.plot(ax=ax[0],facecolor='None',edgecolor='#F39C12')

    #ax[1].set_title('B.') #2014 Regular Point Grid within ASM')
    ax[1].scatter(df_total['lon'],df_total['lat'],c='#2980B9',s=0.05)
    df_comp.plot(ax=ax[1],facecolor='None',edgecolor='#F39C12')

    #ax[2].set_title('C.') #Example of Alpha = 0.014 Concave Hull (Good Fit)')
    df_comp.plot(ax=ax[2],facecolor='None',edgecolor='#F39C12')
    ch = concave_hull(points,0.0141,df_comp)
    ch.plot(ax=ax[2],facecolor='None',edgecolor='#2980B9')

    #ax[3].set_title('D.') # Example of Alpha = 0.001 Concave Hull (Bad Fit)')
    df_comp.plot(ax=ax[3],facecolor='None',edgecolor='#F39C12')
    ch2 = concave_hull(points,0.001,df_comp)
    ch2.plot(ax=ax[3],facecolor='None',edgecolor='#2980B9')
