...
def in_area(df):
    ...
    shp = gpd.read_file('data/2014_plus.shp')
    p_shp = shp.to_crs('EPSG:32618')
    buff = unary_union([mp.buffer(200*1000) for mp in p_shp['geometry']])
    print('complete')
...
if __name__ == "__main__":
    ...
    for file in os.listdir(path):
        ...
        if file.endswith('.txt') and any(n in file for n in lookup_year): 
            ...
            if count ==0: 
                ...
                df = df[df['combo'] > 0]
                print(len(df))
                years_df.append(df[['lon','lat','ndmi','year','type']])
    ...
    start = time.time()
    df = segment_fit(df,'ndmi')
    df.to_csv('segmentation_2deriv_ndmi_2022_11_16_try4.txt',mode='a',sep=',')
