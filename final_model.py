...
def concave_hull(points,a,compar):
    ...
    if len(hull) != 0: 
        ...
        #plt.show()
        hull_explode = hull.explode(ignore_index=True)
        
        
        polygon = gpd.GeoDataFrame(index=[0], crs='ESRI:102001', geometry=hull_explode)
...
##    points = [(x,y,) for x, y in zip(mort['lon'],mort['lat'])]
##    points2 = [Point([x,y]) for x, y in zip(mort['lon'],mort['lat'])]
##    mPoint = gpd.GeoDataFrame(crs='ESRI:102001', geometry=points2)
##    mPoint.to_file('rasters/concave_hull/2021_test_p_m.shp', driver='ESRI Shapefile')
##    na_map3 = na_map[na_map['DAM'] == 2]
##    
...
##    plt.show()
##    if len(ch2) > 0: 
##        ch2.to_file('rasters/concave_hull/2021_test_mort.shp', driver='ESRI Shapefile')
